    ...
    public sealed class CalibrationExcelReportService
    {
        ...
        public bool TryGenerate(string jobDirectory, out string reportPath, out string error)
        {
            ...
            try
            {
                ...
                try
                {
                    CreateWorkbook(temporaryPath, summary, task, sampleRows, rawRows, resultRows);
            ...
            bool freezeTopRow)
        {
            ...
            for (int rowIndex = 0; rowIndex < rows.Count; rowIndex++)
            {
                ...
                bool taskLongText = name == "任务快照" && rowIndex > 0 && source.Any(value => value?.Length > 18);
                Row row = new() { RowIndex = (uint)(rowIndex + 1), Height = rowIndex == 0 ? 28 : taskLongText ? 38 : 20, CustomHeight = true };
        ...
        private static void ValidateWorkbook(string path)
        {
            ...
            string[] required = { "任务与结果", "正式采样", "原始通道", "任务快照" };
